...
def negative_assert_symbol(first_name):
    user_body = get_user_body(first_name)
    #Получаю в переменную user_body изменённое тело для запроса на создание пользователя

    response = sender_stand_request.post_new_user(user_body)
    #сохранил рузультат запроса

    assert response.status_code == 400
    # Проверяется, что код ответа равен 200

    assert response.json()["code"] == 400
    # Проверяется, что в теле ответа присутствует 400

    assert response.json()["message"] == "Имя пользователя введено некорректно." \
                                         " Имя может содержать только русские или латинские буквы," \
                                         " длина должна быть не менее 2 и не более 15 символов"
    # В теле ответа есть код "Имя пользователя введено некорректно.\
    # Имя может содержать только русские или латинские буквы, длина \
    # должна быть не менее 2 и не более 15 символов"
